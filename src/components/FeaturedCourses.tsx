        ...
        <div className='grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8 justify-center'>
          {featuredCourses.map((course:Course) => (
            <div key={course.id} className='bg-white rounded-lg shadow-lg p-6'>
              ...
              <div className='mt-6'>
                <Link href={`/courses/${course.slug}`} className='px-4 py-2 rounded border border-neutral-600 text-neutral-700 bg-white hover:bg-gray-100 transition duration-200'>View Course</Link>
              </div>
            </div>
          ))}
        </div>
